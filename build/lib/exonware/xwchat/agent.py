...
class XWChatAgent(AChatAgent):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if use_google_storage and not self._storage_connection:
            # XWStorage is required
            if storage_connection is None:
                logger.warning(
                    "xwstorage not available. Install with: pip install exonware-xwstorage"
                )
            elif google_storage_config:
    ...
    def _init_google_storage(self, config: dict[str, Any]) -> None:
        ...
        # XWStorage is required
        if storage_connection is None:
            logger.warning("xwstorage not available, cannot initialize Google Storage")
            return
        try:
    ...
    def _init_google_auth(self) -> None:
        ...
        # XWAuth is required
        if auth is None:
            logger.warning("xwauth not available, cannot initialize Google OAuth")
            return
        try:
            ...
            # Create local storage for auth data
            # XWStorage is available
            if storage_connection:
                # Use local storage for auth persistence
                auth_storage_config = {
                    "connector": "local",
                    "format": "json",
